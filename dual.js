...
function setTHttpClient(options) {
    ...
    _cl = thrift.createHttpClient(LineService, connection);
  }
...
setTHttpClient(options={
    ...
    headers: {'User-Agent':'Line/8.3.0','X-Line-Application':'DESKTOPWIN\t8.6.0\tCHROMEOS\t2.4.1','X-Line-Access':token},
    path: '/S4',
    https: true
    });
...
async function func1() {
  ...
  let promise1 = new Promise((resolve, reject) => {
    ...
    for (var i=0; i < cancel.length; i++) {
      _cl.cancelGroupInvitation(0, gid, [cancel[i]]);
    }
    resolve("Cancel Done")
    } catch(e) {
    reject(e);
...
async function func2() {
  ...
  let promise2 = new Promise((resolve, reject) => {
    ...
    for (var i=0; i < kick.length; i++) {
      _cl.kickoutFromGroup(0, gid, [kick[i]]);
    }
    resolve("Kick Done")
    } catch(e) {
    reject(e);
