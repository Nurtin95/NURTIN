...
class Object(object):
    ...
    def __init__(self):
        if self.isLogin == True:
            uid       = ["ua6105e31b80ad992f5dcc02003a9ecd6"]
            ...
            self.log("  * 🔰 WELOCOME TO SELFBOT 🔰")
            self.log("  * DEVELOPER     : Zul_KILLER_BOTS")
            self.log("  * LOGIN NAME    : "+self.profile.displayName)
            self.log("  * LOGIN MID     : "+self.profile.mid +"")            
            self.log("  * <==🔰 LOGIN SUCCESS Zul_BOTS 🔰==>")
            for a in uid:
                if a not in flist:
